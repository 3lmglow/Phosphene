...
| Yukehome、自建 AI 后端、GPT/GLM 调用层 | 后端直接请求 `/mcp` | `token`（默认） |
...
请求时组成 `Authorization: Bearer ${PHOSPHENE_MCP_TOKEN}`。Yukehome 使用的正是这两个
变量名，并会在只填写域名时补齐 `/mcp`。也可以改发
...
旧 SSE 握手的客户端无法直连当前无状态 Endpoint，需要升级客户端；Phosphene 不额外维护
一套即将淘汰的 SSE 路由。
