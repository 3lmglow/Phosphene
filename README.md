...
打开 `http://localhost:3000`。默认数据在 `.data/phosphene`，图片在 `.data/uploads`。本地未配置时 Setup Token 为 `phosphene-local-setup`；完成设置后页面只显示一次 AI Token。
...
3. 不需要必填环境变量。可选设置 `PHOSPHENE_TIMEZONE=Asia/Shanghai`；也可以自己提供至少 24 位的 `PHOSPHENE_SETUP_TOKEN`。
4. 不要设置 `DATABASE_URL`、`S3_*`；`STORAGE_DRIVER` 保持 `local` 或不填。
5. 给服务绑定 Zeabur 域名。Zeabur 会提供 `PORT` 与 `ZEABUR_WEB_URL`，应用会自动识别。
6. 如果没有手动设置 Setup Token，在 Zeabur Runtime Logs 搜索 `setupToken`，复制应用自动生成并保存在 `/data` 的一次性 Token。
7. 打开域名完成首次设置，立即保存只显示一次的 `phosphene_ai_...` AI Token。
...
| `PHOSPHENE_SETUP_TOKEN` | 可选一次性初始化凭证；缺失、占位或过短时自动安全生成并持久化 |
...
- 生产环境会替换缺失、占位或过短的 Setup Token，并将随机值以 `0600` 权限持久化在 `/data`
