...
这是一个由人与 AI 一起完成的 vibe coding 项目。欢迎使用、研究和二次创作；项目仍可能有尚未发现的 bug，也鼓励大家按自己的关系、习惯和部署环境继续修改、修复与完善。如果公开发布修改版，也欢迎注明来源并链接回本仓库。具体授权以 [MIT License](LICENSE) 为准。

## 已实现

- `daily`、`challenge`、`surprise` 三类任务
- daily 一次性或每日重复；重复规则与每日实例分离，暂停会立即移除当天尚未提交的实例，也可恢复和修改未来实例
- easy / medium / hard 难度倍率与不可变积分账本
- self / ai_review 两种确认方式
- 无证据、文字、图片、文字或图片、文字和图片五种证据要求
- 任务详情保留每一次提交的原始文字、图片、审核状态与打回理由
- 图片真实格式与像素检查、Sharp 重新编码、EXIF/GPS 清除、私有审核预览
- 失败/逾期扣 50%，余额不低于 0；额外 AI 扣分有每日上限与 user 暂停开关
- 按时区计算的连击、延迟审核历史补算、总坚持天数和完整统计
- 轨迹页内置奖惩规则说明；历史任务直接显示失败或逾期的实际扣分
- 25 个内置成就
- 两项普适预设、自定义奖励、原子兑换与 AI 履行队列
- 首次设置、Argon2id、服务端会话、CSRF、AI Token 轮换和完整审计日志
- 恰好 7 个 MCP 工具
- 数据库、幂等记录与私有图片的流式 ZIP 导出/恢复界面
...
6. user 按任务要求提交文字、图片或两者；自我确认任务立即结算，AI 审核任务进入“待确认”。若提交被打回，任务详情会保留理由和上次文字，方便修改后再次提交。
...
建议起步资源为 1 vCPU、512 MB 内存。普通私人实例通常够用；大量图片会增加卷容量和备份体积，恢复时还需要足够的临时磁盘空间。
...
AI 打回待确认提交时必须填写具体理由；user 再次打开任务会看到这段说明，并可在上次文字
基础上修改后重新提交。暂停重复 daily 会立刻取消当天尚未提交的实例，使它马上离开今日
待办；已经提交、完成或进入历史的记录不会被撤销。若在当天截止前恢复，因暂停而取消的
当日实例也会恢复为待完成。升级前已经暂停但遗留待办的系列，会依据暂停审计记录自动
清理；自然到期的系列不会被误判。
...
- “惩罚强度”0–5 是提供给 AI 理解边界的偏好信号，不改变服务端固定的 50% 任务扣分公式
...
| 网站 ZIP | 业务表、幂等记录、图片原件与审核预览；不覆盖登录凭证和 AI Token | 应用级导出、迁移和业务时间点恢复 |

至少在每次升级前创建 `/data` 卷快照。删除 Zeabur 服务时要仔细确认是否同时删除卷；删除卷不可恢复。不要在应用运行时只复制 `phosphene.sqlite` 单文件，因为 WAL 中可能还有尚未合并的数据。

网站 ZIP 采用数据库一致性快照、流式下载和磁盘暂存恢复；带图片的备份可以在原实例就地恢复。恢复期间系统会短暂进入维护状态，新业务请求返回 503，健康检查仍保持可用。图片先写入新的私有对象 Key，全部校验和数据库事务成功后才清理旧文件。

网站恢复入口需要当前登录密码，设计上不会覆盖密码、会话或 AI Token。恢复时 ZIP、旧图片和新图片会短暂同时占用 `/data`，开始前应创建卷快照并确认剩余空间。只恢复自己信任且来源明确的 Phosphene 备份，不要上传第三方提供的 ZIP。完整流程、容量估算、格式兼容和失败处理见 [备份与恢复](docs/BACKUP.md)。
...
- [备份与恢复](docs/BACKUP.md)：ZIP 内容、流式恢复、容量估算、校验与演练步骤
...
- AI 主动判定任务失败和 `adjust_points(kind="penalty")` 都服从 `punishments_paused`，并共用每日 AI 扣分上限；系统自动逾期仍按任务规则独立结算
