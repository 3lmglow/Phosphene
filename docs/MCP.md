...
如果 punishments_paused=true，不要调用 manage_task(action="fail") 施加任务失败扣分；
当前版本的这条扣分路径还没有完整复用 adjust_points 的暂停与每日上限检查。
...
- `edit`：只允许待完成任务
- `cancel`：取消，不扣分
- `fail`：判定失败并按任务规则扣分；当前版本存在 [边界检查已知问题](KNOWN_ISSUES.md)
