...
512 MB 是单服务模式的正式最低内存档位。镜像会约束 Node 堆，并在 PGlite 初始化阶段主动回收临时内存；不要把 `PGLITE_INITIAL_MEMORY_MB` 调低到 128 以下。部署后可在 ready 日志查看 `memoryRssMb`，并在 Zeabur Metrics 核对实际峰值。
